...
func (b *Button) BackgroundColor() canvas.Color {
	return b.backgroundColor
}

func (b *Button) SetBackgroundColor(c canvas.Color) {
	b.backgroundColor = c
}

func (b *Button) TextColor() canvas.Color {
	if !b.enabled {
...
func (b *Button) SetTextColor(c canvas.Color) {
	b.textColor = c
}
...
func (b *Button) PreferredSize(self Widget, ctx *canvas.Context) (int, int) {
...
func (b *Button) Draw(self Widget, ctx *canvas.Context) {
	...
	var gradTop canvas.Color
	var gradBot canvas.Color
	...
	bg := canvas.LinearGradient(bx, by, bx, by+bh, gradTop, gradBot)
	ctx.SetFillPaint(bg)
	ctx.Fill()
	...
	if b.icon > 0 || b.imageIcon > 0 {
		...
		if b.icon > 0 {
			...
			imgPaint := canvas.ImagePattern(iconPosX, iconPosY-ih*0.5, iw, ih, 0, b.imageIcon, eOff)
