...
func (i *ImageView) PreferredSize(self Widget, ctx *canvas.Context) (int, int) {
	if i.image == 0 {
		return 0, 0
	}
...
func (i *ImageView) Draw(self Widget, ctx *canvas.Context) {
	if i.image == 0 {
		return
	}
	x := float32(i.x)
	...
	imgPaint := canvas.ImagePattern(x, y, w, h, 0, i.image, 1.0)
