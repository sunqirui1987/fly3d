...
func (v *VScrollPanel) OnPerformLayout(self Widget, ctx *canvas.Context) {
...
func (v *VScrollPanel) PreferredSize(self Widget, ctx *canvas.Context) (int, int) {
	if len(v.children) == 0 {
		return 0, 0
	}
	layout := self.Layout()
...
func (v *VScrollPanel) Draw(self Widget, ctx *canvas.Context) {
	if len(v.children) == 0 {
		return
	}
	x := float32(v.x)
	...
	if v.childPreferredHeight > v.h {
		...
		paint := canvas.BoxGradient(x+w-12+1, y+4+1, 8, h-8, 3, 4, canvas.MONO(0, 32), canvas.MONO(0, 92))
		ctx.BeginPath()
		ctx.RoundedRect(x+w-12, y+4, 8, h-8, 3)
		ctx.SetFillPaint(paint)
		ctx.Fill()

		barPaint := canvas.BoxGradient(x+y-12-1, y+4+1+(h-8-scrollH)*v.scroll-1, 8, scrollH, 3, 4, canvas.MONO(220, 100), canvas.MONO(128, 100))
		ctx.BeginPath()
