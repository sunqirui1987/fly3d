...
func NewColorWheel(parent Widget, colors ...canvas.Color) *ColorWheel {
...
func (c *ColorWheel) SetCallback(callback func(color canvas.Color)) {
	c.callback = callback
}

func (c *ColorWheel) Color() canvas.Color {
	return canvas.HSL(c.hue, c.saturation, c.lightness)
}

func (c *ColorWheel) SetColor(color canvas.Color) {
...
func (c *ColorWheel) PreferredSize(self Widget, ctx *canvas.Context) (int, int) {
	return 100, 100
}

func (c *ColorWheel) Draw(self Widget, ctx *canvas.Context) {
	...
	for i := 0; i < 6; i++ {
		a0 := float32(i)/6.0*canvas.PI*2.0 - aeps
		a1 := float32(i+1)/6.0*canvas.PI*2.0 + aeps
		ctx.BeginPath()
		ctx.Arc(cx, cy, r0, a0, a1, canvas.Clockwise)
		ctx.Arc(cx, cy, r1, a1, a0, canvas.CounterClockwise)
		ctx.ClosePath()
		...
		color1 := canvas.HSLA(a0/(canvas.PI*2), 1.0, 0.55, 255)
		color2 := canvas.HSLA(a1/(canvas.PI*2), 1.0, 0.55, 255)
		paint := canvas.LinearGradient(ax, ay, bx, by, color1, color2)
	...
	ctx.SetStrokeColor(canvas.MONO(0, 64))
	ctx.Stroke()
	...
	ctx.SetStrokeColor(canvas.MONO(255, 192))
	ctx.Stroke()

	paint := canvas.BoxGradient(r0-3, -5, r1-r0+6, 10, 2, 4, canvas.MONO(0, 128), canvas.MONO(0, 0))
	ctx.BeginPath()
	ctx.Rect(r0-2-10, -4-10, r1-r0+4+20, 8+20)
	ctx.Rect(r0-2, -4, r1-r0+4, 8)
	ctx.PathWinding(canvas.Hole)
	ctx.SetFillPaint(paint)
	ctx.Fill()

	// Center triangle
	r := r0 - 6
	sin1, cos1 := sinCosF(120.0 / 180.0 * canvas.PI)
	sin2, cos2 := sinCosF(-120.0 / 180.0 * canvas.PI)
	ax := cos1 * r
	ay := sin1 * r
	bx := cos2 * r
	by := sin2 * r
	...
	ctx.LineTo(bx, by)
	ctx.ClosePath()
	triPaint1 := canvas.LinearGradient(r, 0, ax, ay, canvas.HSL(c.hue, 1.0, 0.5), canvas.MONO(255, 255))
	ctx.SetFillPaint(triPaint1)
	ctx.Fill()
	triPaint2 := canvas.LinearGradient((r+ax)*0.5, ay*0.5, bx, by, canvas.MONO(0, 0), canvas.MONO(0, 255))
