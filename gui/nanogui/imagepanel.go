...
func (i *ImagePanel) PreferredSize(self Widget, ctx *canvas.Context) (int, int) {
...
func (i *ImagePanel) Draw(self Widget, ctx *canvas.Context) {
	cols, _ := i.gridSize()
	...
	for j, image := range i.images {
		...
		imgPaint := canvas.ImagePattern(pX+ix, pY+iy, iw, ih, 0, image.ImageID, toF(i.mouseIndex == j, 1.0, 0.7))
		ctx.BeginPath()
		ctx.RoundedRect(pX, pY, thumbSize, thumbSize, 5)
		ctx.SetFillPaint(imgPaint)
		ctx.Fill()

		shadowPaint := canvas.BoxGradient(pX-1, pY, thumbSize+2, thumbSize+2, 5, 3, canvas.MONO(0, 128), canvas.MONO(0, 0))
		...
		ctx.SetStrokeColor(canvas.MONO(255, 80))
		ctx.Stroke()
	}
}
