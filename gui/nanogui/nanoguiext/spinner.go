...
func (s *Spinner) OnPerformLayout(self nanogui.Widget, ctx *canvas.Context) {
	s.filter.SetPosition(s.Parent().AbsolutePosition())
}

func (s *Spinner) PreferredSize(self nanogui.Widget, ctx *canvas.Context) (int, int) {
	return 0, 0
}

func (s *Spinner) Draw(self nanogui.Widget, ctx *canvas.Context) {
}
...
func (sf *SpinnerFilter) PreferredSize(self nanogui.Widget, ctx *canvas.Context) (int, int) {
	if sf.isActive() {
...
func (sf *SpinnerFilter) Draw(self nanogui.Widget, ctx *canvas.Context) {
	if sf.isActive() {
		var py int
		fw, fh := sf.Parent().Size()
		if window, ok := sf.Parent().(*nanogui.Window); ok {
		...
		ctx.Save()
		ctx.BeginPath()
		ctx.SetFillColor(canvas.MONOf(0, alpha))
		...
		if showSpinner {
			...
			for i := 0; i < sf.num; i++ {
				...
				ctx.SetStrokeColor(canvas.MONOf(1.0, float32(i)/float32(sf.num)))
				ctx.Stroke()
				rotation += dr
			}
		}
