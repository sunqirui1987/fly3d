...
func (s *Slider) HighlightColor() canvas.Color {
	return s.highlightColor
}

func (s *Slider) SetHighlightColor(c canvas.Color) {
	s.highlightColor = c
}
...
func (s *Slider) PreferredSize(self Widget, ctx *canvas.Context) (int, int) {
	return 70, 12
}

func (s *Slider) Draw(self Widget, ctx *canvas.Context) {
	sx := float32(s.x)
	sy := float32(s.y)
	sw := float32(s.w)
	sh := float32(s.h)
	...
	background := canvas.BoxGradient(sx, cy-3+1, sw, 6, 3, 3, canvas.MONO(0, a1), canvas.MONO(0, a2))
	...
	knobShadow := canvas.RadialGradient(kx, ky, kr-3, kr+3, canvas.MONO(0, 64), s.theme.Transparent)
	ctx.BeginPath()
	...
	knobPaint := canvas.LinearGradient(sx, cy-kr, sx, cy+kr, s.theme.BorderLight, s.theme.BorderMedium)
	knobReversePaint := canvas.LinearGradient(sx, cy-kr, sx, cy+kr, s.theme.BorderMedium, s.theme.BorderLight)
	...
	ctx.SetFillColor(canvas.MONO(150, a3))
	ctx.Stroke()
	ctx.Fill()
}
