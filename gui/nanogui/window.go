...
func (w *Window) PreferredSize(self Widget, ctx *canvas.Context) (int, int) {
	if w.buttonPanel != nil {
...
func (w *Window) OnPerformLayout(self Widget, ctx *canvas.Context) {
	if w.buttonPanel == nil {
...
func (w *Window) Draw(self Widget, ctx *canvas.Context) {
	...
	// Draw a drop shadow
	shadowPaint := canvas.BoxGradient(wx, wy, ww, wh, cr*2, ds*2, w.theme.DropShadow, w.theme.Transparent)
	ctx.BeginPath()
	...
	if w.title != "" {
		headerPaint := canvas.LinearGradient(wx, wy, ww, wh+hh, w.theme.WindowHeaderGradientTop, w.theme.WindowHeaderGradientBot)
