...
func (t *TextBox) PreferredSize(self Widget, ctx *canvas.Context) (int, int) {
...
func (t *TextBox) Draw(self Widget, ctx *canvas.Context) {
	...
	bg := canvas.BoxGradient(x+1, y+2, w-2, h-2, 3, 4, canvas.MONO(255, 32), canvas.MONO(32, 32))
	fg1 := canvas.BoxGradient(x+1, y+2, w-2, h-2, 3, 4, canvas.MONO(150, 32), canvas.MONO(32, 32))
	fg2 := canvas.BoxGradient(x+1, y+2, w-2, h-2, 3, 4, canvas.RGBA(255, 0, 0, 100), canvas.RGBA(255, 0, 0, 50))
	...
	ctx.SetStrokeColor(canvas.MONO(0, 48))
	ctx.Stroke()
	...
	if t.unitImage > 0 {
		...
		imgPaint := canvas.ImagePattern(x+w-xSpacing-unitWidth, drawPosY-unitHeight*0.5,
		...
		unitWidth, _ = ctx.TextBounds(0, 0, t.units)
		ctx.SetFillColor(canvas.MONO(255, toB(t.enabled, 64, 32)))
		ctx.SetTextAlign(canvas.AlignRight | canvas.AlignMiddle)
		ctx.Text(x+w-xSpacing, drawPosY, t.units)
	}

	switch t.alignment {
	case TextLeft:
		ctx.SetTextAlign(canvas.AlignLeft | canvas.AlignMiddle)
		drawPosX += xSpacing
	case TextRight:
		ctx.SetTextAlign(canvas.AlignRight | canvas.AlignMiddle)
		drawPosX += w - unitWidth - xSpacing
	case TextCenter:
		ctx.SetTextAlign(canvas.AlignCenter | canvas.AlignMiddle)
		drawPosX += w * 0.5
	}
	if t.enabled {
	...
	if t.committed {
		...
		if len(t.preeditText) != 0 {
			...
			ctx.SetStrokeColor(canvas.MONO(255, 160))
			ctx.SetFillColor(canvas.MONO(255, 80))
		...
		if caretX > 0 {
			...
			ctx.SetStrokeColor(canvas.RGBA(255, 192, 0, 255))
...
func (t *TextBox) updateCursor(ctx *canvas.Context, lastX float32, glyphs []canvas.GlyphPosition) {
...
func (t *TextBox) textIndex2Position(index int, lastX float32, glyphs []canvas.GlyphPosition) float32 {
	if index == len(glyphs) {
		return lastX
	}
...
func (t *TextBox) position2CursorIndex(posX, lastX float32, glyphs []canvas.GlyphPosition) int {
	cursorIndex := 0
	if len(glyphs) == 0 {
		return 0
	}
