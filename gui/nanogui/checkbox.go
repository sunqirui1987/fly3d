...
func (c *CheckBox) PreferredSize(self Widget, ctx *canvas.Context) (int, int) {
	fw, fh := c.FixedSize()
	if fw > 0 || fh > 0 {
		return fw, fh
	}
...
func (c *CheckBox) Draw(self Widget, ctx *canvas.Context) {
	cx := float32(c.x)
	cy := float32(c.y)
	ch := float32(c.h)
	...
	bgPaint := canvas.BoxGradient(cx+1.5, cy+1.5, ch-2.0, ch-2.0, 3, 3, canvas.MONO(0, bgAlpha), canvas.MONO(0, 180))
	ctx.BeginPath()
