...
func (p *PopupButton) Draw(self Widget, ctx *canvas.Context) {
	...
	if p.chevronIcon != 0 {
		...
		ctx.SetTextAlign(canvas.AlignMiddle | canvas.AlignLeft)
...
func (p *PopupButton) PreferredSize(self Widget, ctx *canvas.Context) (int, int) {
	w, h := p.Button.PreferredSize(self, ctx)
	return w + 15, h
}

func (p *PopupButton) OnPerformLayout(self Widget, ctx *canvas.Context) {
