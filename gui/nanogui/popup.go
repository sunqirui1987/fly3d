...
func (p *Popup) OnPerformLayout(self Widget, ctx *canvas.Context) {
...
func (p *Popup) Draw(self Widget, ctx *canvas.Context) {
	...
	/* Draw a drop shadow */
	shadowPaint := canvas.BoxGradient(px, py, pw, ph, cr*2, ds*2, p.theme.DropShadow, p.theme.Transparent)
	ctx.BeginPath()
